...
public class CraftingTable implements Listener {
	...
	public void prepareCrafting(PrepareItemCraftEvent event) {
		...
		if (event.isRepair() && !CustomItemUtils.isCustomItem(inventory.getResult())) {
			/*
			...
			if (items.size() == 2) {
				...
				if (CustomItemUtils.isCustomItem(toCombine[0]) && CustomItemUtils.isCustomItem(toCombine[1])) {
	...
	// inventory.setResult(CustomItemUtils.getCustomItemStack(cItem.getIdName()).getItemStack());
	// return;
	// }
	// }
	// }
