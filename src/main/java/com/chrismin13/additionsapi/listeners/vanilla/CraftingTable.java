...
public class CraftingTable implements Listener {
	...
	public void prepareCrafting(PrepareItemCraftEvent event) {
		...
		if (AdditionsAPI.isCustomItem(inventory.getResult())) {

			CustomItem cItem = new CustomItemStack(inventory.getResult()).getCustomItem();
			ItemPermissions perm = cItem.getPermissions();

			for (HumanEntity human : event.getViewers()) {
				if (!PermissionUtils.allowedAction((Player) human, perm.getType(), perm.getCraft())) {
